...
class Polc extends React.Component {
  ...
  renderCommitment = (commitment, index) => {
    const { lockStart, lockEnd, availableValue, lockedValue } = commitment.value;

    return (
      <List.Item>
        <Card actions={[
          <Icon type="export" title="Withdraw Fund" data-index={index} onClick={this.withdrawFund} />,
          <Icon type="dollar" title="Withdraw Reward" data-index={index} onClick={this.withdrawReward}/>]
        }>
          <Row>
            <Col span={12}>
              <Statistic title="Lock Start" value={formatDayTime(lockStart)} />
            </Col>
            <Col span={12}>
              <Statistic title="Lock End" value={formatDayTime(lockEnd)} />
            </Col>
            <Col span={12}>
              <Statistic title="Locked Value" value={formatEthValue(lockedValue)} />
            </Col>
            <Col span={12}>
              <Statistic title="Available Value" value={formatEthValue(availableValue)} />
            </Col>
          </Row>
        </Card>
      </List.Item>
  ...
  render() {
    ...
    return (
      <Card
      title="PoLC"
      extra={<Button type="primary" onClick={this.toggleModal}>Commit fund</Button>}
    >
      {this.renderCommiments()}
      <CommimentForm visible={isModalVisible} onClose={this.toggleModal}/>
    </Card>
    );
  }
}
...
function mapStateToProps(state) {
  const { contracts, drizzleStatus } = state
