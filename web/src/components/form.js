...
class CustomizeForm extends React.Component {
  ...
  renderFormItems = () => {
    ...
    return _.map(items, (item) => {
      const { field = 'input', fieldOptions, initialValue, label, name, rules } = item
      ...
      return (
        <FormItem key={name} {...formItemLayout} label={!label ? _.capitalize(name) : label}>
          {getFieldDecorator(name, decoratorOptions)(<Field {...fieldOptions} />)}
        </FormItem>
      )
    })
  }
